...
class LoginProvider extends ChangeNotifier with Utility {
  ...
  Future<void> login(
    ...
    if (_isConnected) {
      try {
        loginResponse = await logInRepository.login(userName: userName, password: password);
        if ((loginResponse.toString().isNotEmpty) && (loginResponse.errorMessage?.isEmpty ?? false)) {
          ...
          showToast(context: context, msg: loginResponse.errorMessage ?? "حدث خطأ ما يرجى المحاولة لاحقا", state: false);
        }
        // }
      } catch (error) {
        showToast(
